...
func TestUsernameDomainWorkstation(t *testing.T) {
	...
	tables := []struct {
		...
		{"username without domain and empty workstation", username, "", username, "", []byte{
			0x4e, 0x54, 0x4c, 0x4d, 0x53, 0x53, 0x50, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x82, 0x08, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00}},
		{"username with domain and empty workstation", domain + "\\" + username, "", username, domain, []byte{
			0x4e, 0x54, 0x4c, 0x4d, 0x53, 0x53, 0x50, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0x92, 0x08, 0x00,
			0x08, 0x00, 0x08, 0x00, 0x28, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x30, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4d, 0x59, 0x44, 0x4f, 0x4d, 0x41, 0x49, 0x4e}},
		{"username with domain and non-empty workstation", domain + "\\" + username, workstation, username, domain, []byte{
			0x4e, 0x54, 0x4c, 0x4d, 0x53, 0x53, 0x50, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0xb2, 0x08, 0x00,
			0x08, 0x00, 0x08, 0x00, 0x28, 0x00, 0x00, 0x00, 0x04, 0x00, 0x04, 0x00, 0x30, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4d, 0x59, 0x44, 0x4f, 0x4d, 0x41, 0x49, 0x4e,
			0x4d, 0x59, 0x50, 0x43}},
		{"username without domain and non-empty workstation", username, workstation, username, "", []byte{
			0x4e, 0x54, 0x4c, 0x4d, 0x53, 0x53, 0x50, 0x00, 0x01, 0x00, 0x00, 0x00, 0x05, 0xa2, 0x08, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x28, 0x00, 0x00, 0x00, 0x04, 0x00, 0x04, 0x00, 0x28, 0x00, 0x00, 0x00,
			0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4d, 0x59, 0x50, 0x43}},
	}
	...
	for _, table := range tables {
		...
		if tdomain != table.xd {
			...
			if !bytes.Equal(tb, table.xb) {
				t.Fatalf("negotiate message bytes not correct, expected\n%v got\n%v",
					hex.EncodeToString(table.xb), hex.EncodeToString(tb))
			}
		})
	}
...
func TestCalculateNTLMv2Response(t *testing.T) {
	...
	v, _ := computeNtlmV2Response(NTLMv2Hash, challenge, ClientChallenge, Time, targetInfo)
